...
const autoContentType = (req, file, cb) => {
    file.stream.once('data', async (firstChunk) => {
        var type = filetypemime(firstChunk);
        var mime;
...
class S3Storage {
    constructor(opts) {
        ...
        this.transformUpload = (opts, req, file, cb) => {
            var storage = this;
            var results = [];
            parallel(storage.getTransforms.map((transform) => transform.key.bind(storage, req, file)), (err, keys) => {
                if (err)
                    return cb(err);
                keys.forEach((key, i) => {
                    var currentSize = 0;
        ...
        this._removeFile = (req, file, cb) => {
            ...
            const deleteObjectCommand = new DeleteObjectCommand(deleteCommandInput);
