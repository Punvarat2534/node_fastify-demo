...
const defaultKey = (req, file, cb) => {
    node_crypto_1.default.randomBytes(16, function (err, raw) {
        cb(err, err ? undefined : raw.toString('hex'));
    });
};
const autoContentType = (req, file, cb) => {
    file.stream.once('data', async (firstChunk) => {
        var type = (0, magic_bytes_js_1.filetypemime)(firstChunk);
        var mime;
        if (type && type.length > 0) {
            mime = type[0];
        }
        else if ((0, is_svg_1.default)(firstChunk.toString())) {
            mime = 'image/svg+xml';
        }
        else {
            mime = 'application/octet-stream';
        }
        var outStream = new node_stream_1.default.PassThrough();
...
class S3Storage {
    constructor(opts) {
        ...
        this.directUpload = (opts, file, cb) => {
            ...
            var upload = new lib_storage_1.Upload({ client: this.s3, params });
        ...
        this.transformUpload = (opts, req, file, cb) => {
            var storage = this;
            var results = [];
            (0, run_parallel_1.default)(storage.getTransforms.map((transform) => transform.key.bind(storage, req, file)), (err, keys) => {
                if (err)
                    return cb(err);
                keys.forEach((key, i) => {
                    var currentSize = 0;
        ...
        this._removeFile = (req, file, cb) => {
            ...
            const deleteObjectCommand = new client_s3_1.DeleteObjectCommand(deleteCommandInput);
...
exports.DEFAULT_CONTENT_TYPE = defaultContentType;
